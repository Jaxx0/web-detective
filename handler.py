...
def document_crawler(event, context):
    """This function receives a URL as an argument from the API gateway (query?=url input ) and passes it to the
    crawler function It then stores the response to an s3 bucket an a DynamoDB and returns the extracted title and
    the S3 URL of the stored response object """
    try:
...
def post_url_and_identity(event, context):
    """ This Lambda function receives a URL as an argument (query=<url>) , creates an identifier for the request, stores the URL in a
    ...
    try:
        if event['httpMethod'] == 'GET' and event['queryStringParameters']['query']:
            ...
            db_store = save_to_db(identifier, url,
                                  table=os.environ['URL_TABLE_NAME'])  # saves url keyed to the identifier
